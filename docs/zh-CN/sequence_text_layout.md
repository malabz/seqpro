...
[English](../sequence_text_layout.md)
...
所有输入区间均为半开区间。原始区间表示为
`[sequence_start_position, sequence_end_position)`，要求
`sequence_start_position < sequence_end_position <= sequence_length`。文本区间使用
`text_start_position` 和 `text_length` 表示，要求 `text_length > 0` 且算术不溢出。
...
构造函数参数 `selected_sequence_order` 为空时使用全部 FAI 记录顺序。非空时只选择指定 ID，
并严格按传入顺序布局。ID 必须存在且唯一。未选择序列不能添加排除区间，也不参与任何
坐标映射。只读属性访问器仍为 `sequence_order()`。
...
前缀和与全局文本前缀。run 容量、容器 `size_t` 转换、generation 和所有 64 位前缀都采用
checked arithmetic。所有新索引先在临时容器完成，成功后一次 swap。失败时保持 dirty，
不发布部分状态。成功时 generation 增加；即使新区间与已有区间完全重合，只要经历 dirty
状态也保守增加。clean 状态重复调用是 no-op。
...
- `Materialize()` 一次分配精确长度，返回 `sequence_text_bytes` 和当前 generation，不建立
  隐藏缓存。
- `CopyTextTo()` 要求非空 `destination_buffer` 且 `destination_size_bytes` 严格等于
  `text_size()`。
- `WriteTo()` 使用固定传输缓冲区流式输出。

`MaterializedSequenceText::sequence_text_bytes` 包含最终 NUL；必须使用
`sequence_text_bytes.size()`，禁止使用 `strlen()`。FASTA active 区域若包含 `0x00` 或
`0x01`，文本访问抛出 `kUnsupportedFileFormat`。库不转义、不替换，也不扫描被排除区间中的
保留字节。
